...
import { deleteFromCart } from '../components/redux/actions/deleteFromCart';
import {  addToCart } from '../components/redux/actions/addToCart';
import { removeFromCart } from '../components/redux/actions/removeFromCart';


function Cart({ items, total, onIncrease, onDecrease, onDelete }) {
  return (
...
<path d="M4.78002 4.99999H16.3334L15.2134 10.5933C15.1524 10.9003 14.9854 11.176 14.7417 11.3722C14.4979 11.5684 14.1929 11.6727 13.88 11.6667H6.83335C6.50781 11.6694 6.1925 11.553 5.94689 11.3393C5.70128 11.1256 5.54233 10.8295 5.50002 10.5067L4.48669 2.82666C4.44466 2.50615 4.28764 2.21182 4.04482 1.99844C3.80201 1.78505 3.48994 1.66715 3.16669 1.66666H1.66669" stroke="black" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
     ...
     {items && items.map((item, i) => {
       const {id, name,img,description, price, count } = item;
       return (
        ...
        <span className='number'>{i+1}</span>
         ...
         <button className="minus-btn" type="button" name="button" onClick={()=> {onDecrease(id)}}>
               <img src="https://designmodo.com/demo/shopping-cart/minus.svg" alt="" />
             </button>
             <span className='count'>{count}</span>
           <button className="plus-btn" type="button" name="button" onClick={()=> {onIncrease(id)}}>
             <img src="https://designmodo.com/demo/shopping-cart/plus.svg" alt="" />
           </button>
          </div>
         <div className="total-price">{`${price} руб.`}</div>
      ...
      <div className='total'>{total > 0 ?  `Общая сумма: ${total} руб` : 'Вы ничего не добавили :('}</div>
    </div>
  )
